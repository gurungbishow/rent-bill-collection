...
const getStatusBadge = (status: string, t: any) => {
  switch (status) {
    ...
    default:
      return (
        <span className="inline-flex items-center text-[10px] sm:text-xs font-semibold uppercase px-2.5 py-0.5 rounded-full bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200/80 dark:border-rose-800/60 tracking-wider">
          {t.unpaid}
        </span>
      );
  }
...
function ItemizedBillCard({ bill, roomName, t, formatDate, formatMoney, formatNumber, language }: any) {
  ...
  return (
    <div id={`bill-receipt-${bill.id}`} className="bg-white dark:bg-slate-900 rounded-2xl p-0.5">
      <Card className="border border-slate-200/80 dark:border-slate-800/80 shadow-xs bg-white dark:bg-slate-900 overflow-hidden rounded-2xl transition-all">
        {/* ── CARD HEADER ── */}
        <div 
          ...
          <div className="p-3.5 sm:p-4 space-y-3.5 text-xs">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-[10px] sm:text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">
            ...
            <div className="border-t border-slate-100 dark:border-slate-800/80 pt-3 grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-0.5 text-right sm:text-left">
                <span className="text-[10px] sm:text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1 justify-end sm:justify-start">
                  <History size={11} className="text-amber-500 shrink-0" />
                  {t.previous_balance || 'Previous Balance'}
                </span>
                <span className={`font-semibold text-xs sm:text-sm ${Number(bill.previous_balance) > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-900 dark:text-slate-100'}`}>
            ...
            <div className="border-t border-slate-100 dark:border-slate-800/80 pt-3 grid grid-cols-2 sm:grid-cols-3 gap-3 bg-slate-50/70 dark:bg-slate-800/40 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800/60">
              <div className="flex flex-col gap-0.5">
                ...
                <span className="font-bold text-xs sm:text-base text-blue-600 dark:text-blue-400">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <span className="font-bold text-xs sm:text-base text-emerald-600 dark:text-emerald-400">
              ...
              <div className="flex flex-col gap-0.5 col-span-2 sm:col-span-1 border-t sm:border-t-0 border-slate-200/60 dark:border-slate-800/60 pt-2 sm:pt-0">
                ...
                <span className={`font-bold text-xs sm:text-base ${Number(bill.remaining_balance) > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-slate-900 dark:text-slate-100'}`}>
            ...
            <div className="pt-1 action-footer-ignore">
              <Button 
                ...
                className="w-full bg-gradient-to-r from-emerald-600 via-teal-500 to-emerald-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl h-9.5 text-xs shadow-xs transition-all font-semibold cursor-pointer border-0 flex items-center justify-center gap-2" 
              >
                {isDownloading ? (
                  <>
